...
ds = HashHopGenerator(max_tokens=max_tokens, batch_size=micro_batch_size, hash_len=hash_len, max_hops=max_hops, cot=True)
#ds = CopyGenerator(max_tokens=max_tokens, batch_size=batch_size, vocab_size=vocab_size)
loader = torch.utils.data.DataLoader(ds, batch_size=None, num_workers=8, pin_memory=True, generator=g)
iter_ = iter(loader)

tokens_per_iter = total_batch_size * max_tokens
...
try:
    for iter in range(start_iter, num_iters):
        ...
        for micro_step in range(grad_acc_steps):
            ...
            with dtype_ctx:
                loss = model(x, y, prompt_len=prompt_len)
